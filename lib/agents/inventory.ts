...
export function runInventoryAgent(context: AgentContext): InventoryAgentOutput {
  const { inventoryContext, metrics, evidenceRefs } = context;
  ...
  if (inventoryContext.requested && !inventoryContext.available) {
    return InventoryAgentOutputSchema.parse({
      ...
      reasoning: inventoryContext.warningMessage ?? "AMI continued with market, competitor, and supplier signals only.",
      confidence: 0.52,
      riskLevel: "medium",
  ...
  if (!inventoryContext.requested) {
    return InventoryAgentOutputSchema.parse({
      ...
      reasoning: "AMI treated inventory as optional and used market, competitor, and supplier evidence.",
      confidence: 0.62,
  ...
  return InventoryAgentOutputSchema.parse({
    ...
    reasoning: `Inventory risk is ${metrics.inventoryRisk}/100 with estimated margin ${metrics.estimatedMargin.toFixed(1)}%.`,
